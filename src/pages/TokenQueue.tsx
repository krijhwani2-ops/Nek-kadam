            ...
            {tokens.map(t => (
              <div key={t.id} className="px-4 py-3 flex items-center justify-between gap-3">
                ...
                <div className="flex gap-1 shrink-0">
                  {t.status === 'WAITING' && (
                    <>
                      <button aria-label="Start" title="Start" disabled={!!busy} onClick={() => doAction('start', { tokenId: t.id, departmentId: t.currentDepartmentId, userId: session?.userId })}
                        className="p-1.5 rounded-lg bg-amber-50 text-amber-600 active:bg-amber-100"><Play size={14} /></button>
                      <button aria-label="Skip" title="Skip" disabled={!!busy} onClick={() => doAction('skip', { tokenId: t.id, userId: session?.userId })}
                        className="p-1.5 rounded-lg bg-slate-50 text-slate-500 active:bg-slate-100"><SkipForward size={14} /></button>
                      {t.priority === 'NORMAL' && (
                        <button aria-label="Urgent" title="Urgent" disabled={!!busy} onClick={() => doAction('priority', { tokenId: t.id, priority: 'URGENT', userId: session?.userId })}
                  ...
                  {t.status === 'IN_PROGRESS' && (
                    <>
                      <button aria-label="Move/Complete" title="Move/Complete" disabled={!!busy} onClick={() => doAction('move', { tokenId: t.id, userId: session?.userId })}
                  ...
                  {t.status === 'SKIPPED' && (
                    <button aria-label="Re-queue" title="Re-queue" disabled={!!busy} onClick={() => doAction('requeue', { tokenId: t.id, userId: session?.userId })}
                      className="p-1.5 rounded-lg bg-blue-50 text-blue-600 active:bg-blue-100"><Undo2 size={14} /></button>
                  )}
                  {!['DONE', 'CANCELLED'].includes(t.status) && (
                    <button aria-label="Cancel" title="Cancel" disabled={!!busy} onClick={() => { if (confirm('Cancel this token?')) doAction('cancel', { tokenId: t.id, userId: session?.userId }); }}
