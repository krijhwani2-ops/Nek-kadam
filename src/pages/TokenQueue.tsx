            ...
            {tokens.map(t => (
              <div key={t.id} className="px-4 py-3 flex items-center justify-between gap-3">
                ...
                <div className="flex gap-1 shrink-0">
                  {t.status === 'WAITING' && (
                    <>
                      <button title="Start" aria-label="Start Token" disabled={!!busy} onClick={() => doAction('start', { tokenId: t.id, departmentId: t.currentDepartmentId, userId: session?.userId })}
                        className="p-1.5 rounded-lg bg-amber-50 text-amber-600 active:bg-amber-100"><Play size={14} /></button>
                      <button title="Skip" aria-label="Skip Token" disabled={!!busy} onClick={() => doAction('skip', { tokenId: t.id, userId: session?.userId })}
                  ...
                  {t.status === 'IN_PROGRESS' && (
                    <>
                      <button title="Move/Complete" aria-label="Move or Complete Token" disabled={!!busy} onClick={() => doAction('move', { tokenId: t.id, userId: session?.userId })}
                  ...
                  {t.status === 'SKIPPED' && (
                    <button title="Re-queue" aria-label="Re-queue Token" disabled={!!busy} onClick={() => doAction('requeue', { tokenId: t.id, userId: session?.userId })}
