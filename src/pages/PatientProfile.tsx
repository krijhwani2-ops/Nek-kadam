...
import { Phone, CreditCard, Plus, Clock, Trash2, X, Printer, FileText, Calendar, Stethoscope, RefreshCw, ScanFace } from 'lucide-react';
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                ...
                <div className="flex gap-1.5">
                  {medicineGroups.length > 1 && (
                    <button type="button" onClick={() => removeGroup(activeGroupIndex)} className="p-2 bg-red-50 dark:bg-red-950/20 text-red-500 dark:text-red-400 rounded-xl hover:bg-red-500 dark:hover:bg-red-600 hover:text-white transition-all border border-red-100 dark:border-red-900/30 shadow-sm" aria-label="Remove medication group">
                      <Trash2 size={16} />
                    </button>
                  )}
                  <button type="button" onClick={addGroup} className="p-2 bg-emerald-50 dark:bg-emerald-950/20 text-emerald-500 dark:text-emerald-400 rounded-xl hover:bg-emerald-500 dark:hover:bg-emerald-600 hover:text-white transition-all border border-emerald-100 dark:border-emerald-900/30 shadow-sm" aria-label="Add medication group">
                    <Plus size={16} />
                  </button>
                </div>
              </div>
                      ...
                      medicineGroups[activeGroupIndex].meds.map(med => (
                        <div key={med.code} className="flex items-center justify-between bg-white dark:bg-slate-800 border border-emerald-100 dark:border-emerald-900/50 px-4 py-2.5 rounded-xl shadow-sm group">
                          ...
                          <button type="button" onClick={() => {
                              ...
                              setMedicineGroups(updated);
                          }} className="p-2 text-slate-300 hover:text-red-500 transition-colors" aria-label="Remove medicine">
                            <Trash2 size={16} />
                          </button>
                        </div>
                      ))
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex gap-1.5">
                        {editVisitMeds.length > 1 && (
                          <button type="button" onClick={() => removeEditGroup(editActiveGroupIndex)} className="p-2 bg-red-50 dark:bg-red-900/20 text-red-500 dark:text-red-400 rounded-lg hover:bg-red-500 dark:hover:bg-red-600 hover:text-white transition-all" aria-label="Remove edit group">
                            <Trash2 size={14} />
                          </button>
                        )}
                        <button type="button" onClick={addEditGroup} className="p-2 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-500 dark:text-emerald-400 rounded-lg hover:bg-emerald-500 dark:hover:bg-emerald-600 hover:text-white transition-all" aria-label="Add edit group">
                          <Plus size={14} />
                        </button>
                      </div>
                    </div>
