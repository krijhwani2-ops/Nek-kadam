...
function parseDateDoctor(cell) {
  ...
  // 1. Try standard date pattern with slashes/dashes/dots (e.g. 21/06/2026, 23/11/25, 7/12/2025)
  let m = raw.match(/(\d{1,2})[\/\-.](\d{1,2})[\/\-.](\d{2,4})/);
  if (!m) {
    // 2. Try date missing slash (e.g. 08/032026 -> 08, 03, 2026)
    m = raw.match(/(\d{1,2})[\/\-.](0[1-9]|1[0-2])(\d{4})/);
  }
  ...
  const docMatch = raw.match(/Dr\.?\s*([A-Za-z]+)/i);
  if (docMatch) {
    doctor = `Dr. ${docMatch[1]}`;
  }
